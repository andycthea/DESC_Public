...
eq_step = load("data/eq_step_initial.h5")
...
for i in range(4):
    ...
    # obj_curr_scale = ObjectiveFromUser(obj_curr, eq_step, target=0.)
    ...
    constraints = (
        ...
        FixCurrent(eq=eq_step),
        ForceBalance(eq=eq_step),
    )

    optimizer = Optimizer("proximal-lsq-exact")
    [eq_step, eq_vac_shadow], result = optimizer.optimize(
    ...
    eq_step.save(f"data/eq_step_opt4_{i}.h5")
    eq_vac_shadow.save(f"data/eq_step_vac_opt4_{i}.h5")

    with open("data/result4_{i}.pickle", "wb") as f:
